...
import { query, domify, attr } from 'min-dom';
...
function getSelectBox(node) {
	return query(selector, node.parentElement);
}
...
module.exports = function (element, definition, bpmnFactory, options) {
	...
	entries.push({
		...
		html: '<div class="bpp-row bpp-select">' +
			'<label for="wf-' + escapeHTML(elementName) + '">' + escapeHTML(label) + '</label>' +
			'<div class="bpp-field-wrapper">' +
			'<select id="wf-' + escapeHTML(elementName) + '" name="selectedElement" data-value>' +
			'</select>' +
