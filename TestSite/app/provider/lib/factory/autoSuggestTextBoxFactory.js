...
import { classes, domify, query } from 'min-dom';

import { assign, find, forEach, debounce } from 'min-dash';
...
var autoSuggestTextBox = function (translate, options, defaultParameters) {
	...
	if (description) {
		query('.bpp-field-wrapper', resource.html).appendChild(entryFieldDescription(translate, description));
	}
	...
	resource.suggestItems = function (element, entryNode, event) {
		...
		function updateSuggestionList(items) {
			var listNode = query('.bpp-autosuggest-list', entryNode);
			...
			classes(listNode).add(CLASS_ACTIVE);
...
function hideSuggestionList(entryNode) {
	var listNode = query('.bpp-autosuggest-list', entryNode);
	classes(listNode).remove(CLASS_ACTIVE);
}
